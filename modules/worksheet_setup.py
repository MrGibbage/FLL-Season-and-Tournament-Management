...
from .constants import (
    ...
    SHEET_RESULTS,
    TABLE_TEAM_LIST,
    TABLE_ROBOT_GAME_AWARDS,
    TABLE_AWARD_DROPDOWNS,
    TABLE_META,
    TABLE_AWARD_DEF,
    TABLE_ROBOT_GAME,
    TABLE_INNOVATION,
...
def set_up_tapi_worksheet(
    tournament: pd.Series, book: Workbook, dfAssignments: pd.DataFrame, using_divisions: bool
) -> bool:
    ...
    if using_divisions:
        ...
        if isinstance(tournament[COL_OJS_FILENAME], float):
            print_error(
                logger,
                f"Invalid OJS filename for tournament {tournament[COL_SHORT_NAME]}: "
...
def set_up_award_worksheet(
    tournament: pd.Series, book: Workbook, dfAwardDef: pd.DataFrame, using_divisions: bool
) -> None:
    ...
    try:
        ...
        print_error(
            logger, f"Invalid robot game award count for {tournament[COL_SHORT_NAME]}: {rg_raw}", e
        )
        rg_awards = 0
    ...
    if rg_awards <= 0:
        logger.warning(
            f"{tournament[COL_SHORT_NAME]} has zero robot game awards allocated; OJS will have an empty Robot Game Awards dropdown."
        )
    else:
        for rg_award_num in range(1, rg_awards + 1):
            ...
            if thisLabel not in label_columns:
                logger.warning(
                    f"Label column '{thisLabel}' not found in AwardDef table, using blank value"
                )
                thisValue = None
            else:
                ...
                try:
                    ...
                    logger.warning(
                        f"Could not find value for {AWARD_COLUMN_ROBOT_GAME} in column {thisLabel}"
                    )
                    thisValue = None

            rg_awards_df.loc[len(rg_awards_df)] = [thisValue]

    # Allow empty RG list but emit warning above; add_table_dataframe will no-op on empty
    ...
    logger.debug(
        f"Added {len(rg_awards_df)} robot game awards and {len(j_awards_df)} judged awards"
    )
...
def set_up_meta_worksheet(
    ...
    scriptfile = os.path.join(tournament_folder, tournament[COL_SHORT_NAME], FILE_CLOSING_CEREMONY)
    ...
    adv_count = _to_int(tournament[COL_ADVANCING])
    if adv_count <= 0:
        logger.warning(
            f"{tournament[COL_SHORT_NAME]} has zero advancing teams configured; please verify tournament settings."
        )
    dfMeta.loc[len(dfMeta)] = {"Key": "Advancing", "Value": adv_count}
...
def add_essential_conditional_formats(book: Workbook, num_teams: int) -> None:
    ...
    try:
        ...
        if award_list_table:
            list_min_col, list_min_row, list_max_col, list_max_row = range_boundaries(
                award_list_table.ref
            )
            ...
            header_rule.dxf = openpyxl.styles.differential.DifferentialStyle(
                fill=bright_green_fill, font=purple_italic_font
            )
        ...
        duplicate_formula = (
            f"COUNTIF(${award_col}${min_row + 1}:${award_col}${max_row},{award_col}{min_row + 1})>1"
        )

        duplicate_rule = Rule(type="expression", formula=[duplicate_formula], stopIfTrue=False)
        duplicate_rule.dxf = openpyxl.styles.differential.DifferentialStyle(
            fill=bright_red_fill, font=purple_italic_font
        )
        ...
        champ_rank_rule.dxf = openpyxl.styles.differential.DifferentialStyle(
            fill=medium_blue_fill, font=white_font
        )
        ...
        advance_count_formula = (
            f'COUNTIF(${advance_col}${min_row + 1}:${advance_col}${max_row},"Yes")=${advance_col}$1'
        )

        advance_header_rule = Rule(
            type="expression", formula=[advance_count_formula], stopIfTrue=False
        )
        advance_header_rule.dxf = openpyxl.styles.differential.DifferentialStyle(
        ...
        row_highlight_rule = Rule(
            type="expression", formula=[row_highlight_formula], stopIfTrue=False
        )
        ...
        for name_obj in book.defined_names.values():
            if hasattr(name_obj, "value") and name_obj.value:
                ...
                if "[" in str(name_obj.value) and "]" in str(name_obj.value):
                    logger.debug(
                        f"Found external reference in defined name: {name_obj.name} = {name_obj.value}"
                    )
                        ...
                        if hasattr(rule, "formula") and rule.formula:
                            for i, formula in enumerate(rule.formula):
                                if formula:
                                    ...
                                    if (
                                        "AwardList!" in formula_str
                                        and "AwardListDropdowns!" not in formula_str
                                    ):
                                        new_formula = formula_str.replace(
                                            "AwardList!", "AwardListDropdowns!"
                                        )
                                        rule.formula[i] = new_formula
                                        logger.debug(
                                            f"Fixed CF formula: {formula} -> {new_formula}"
                                        )
...
def fix_named_ranges(book: Workbook) -> None:
    ...
    try:
        ...
        for name_key, name_obj in book.defined_names.items():
            if (
                hasattr(name_obj, "value")
                and name_obj.value
                and "AwardList!" in str(name_obj.value)
            ):
                old_value = name_obj.value
                name_obj.value = str(name_obj.value).replace("AwardList!", "AwardListDropdowns!")
                logger.debug(
                    f"Updated named range '{name_obj.name}': {old_value} -> {name_obj.value}"
                )
