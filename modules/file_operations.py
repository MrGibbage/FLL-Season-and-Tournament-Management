...
def load_json_without_notes(path: str) -> dict:
    ...
    try:
        ...
        raise json.JSONDecodeError(
            f"Invalid JSON syntax in configuration file: {path}", e.doc, e.pos
        ) from e
...
def copy_files(
    ...
    if COL_OJS_FILENAME not in item or not item[COL_OJS_FILENAME]:
        print_error(
            logger,
            f"Tournament row missing required field '{COL_OJS_FILENAME}' for {item[COL_SHORT_NAME]}",
        )
    ...
    if using_divisions:
        ...
        logger.debug(
            f"Using divisions mode: copying {len(divisions_only_files)} division-specific files"
        )
    else:
        files_to_copy.extend(no_divisions_only_files)
        logger.debug(
            f"Using non-divisions mode: copying {len(no_divisions_only_files)} non-division-specific files"
        )
    ...
    try:
        ...
        new_ojs_file = os.path.join(tournament_folder, item[COL_SHORT_NAME], item[COL_OJS_FILENAME])
...
def generate_tournament_config(
    ...
    # Extract template filenames from season config copy file lists
    template_filenames = {}

    # Get the appropriate file list based on using_divisions
    if using_divisions:
        file_list = config.get("copy_file_list_divisions_only", [])
    else:
    ...
    if existing_config is None:
        ...
        for template_type, filename in template_filenames.items():
            if filename:
                info_section[template_type] = filename
    else:
    ...
    award_columns = [
        col
        for col in tournament.index
        if col.startswith(AWARD_COLUMN_PREFIX_JUDGED) or col == AWARD_COLUMN_ROBOT_GAME
    ]
    ...
    # Check for potential configuration issues with division awards
    if using_divisions:
        div_awards = [a for a in awards_list if a.get("DivAwd", False)]
        tourn_awards = [a for a in awards_list if not a.get("DivAwd", False)]

        if not quiet:
            if div_awards:
                div_names = [a.get("Name", a.get("ID")) for a in div_awards]
                logger.info(f"Division awards ({len(div_awards)}): {', '.join(div_names)}")
            if tourn_awards:
                tourn_names = [a.get("Name", a.get("ID")) for a in tourn_awards]
                logger.info(
                    f"Tournament-level awards ({len(tourn_awards)}): {', '.join(tourn_names)}"
                )

        # Warn if division-enabled tournament has no division awards
        if not div_awards and awards_list:
            warning_msg = (
                f"{tourn_short}: This is a division-enabled tournament but ALL {len(awards_list)} "
                f"awards are marked as tournament-level (DivAwd=FALSE). "
                f"If using division-specific templates, you may need to set DivAwd=TRUE "
                f"for awards in the AwardDef table."
            )
            logger.warning(warning_msg)
            if not quiet:
                from .user_feedback import print_warning

                print_warning(warning_msg)
            award_mismatches.append(warning_msg)

    # Build final config structure
    final_config = {"INFO": info_section, "AWARDS": awards_list}
